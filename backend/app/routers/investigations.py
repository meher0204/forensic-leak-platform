...
from ..models import LeakInvestigation, Image, Recipient, WatermarkedCopy
...
def get_investigation(investigation_id: int, db: Session = Depends(get_db)):
    ...
    inv = (
        db.query(LeakInvestigation)
        .filter(LeakInvestigation.id == investigation_id)
        .first()
    )
    if not inv:
...
def get_investigation_detail(investigation_id: int, db: Session = Depends(get_db)):
